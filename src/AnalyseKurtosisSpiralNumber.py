...
""" kurtosis of spiral number analysis"""
...
def calculate_spiral_number(beads, sim):
    ...
    spiral_number = np.zeros((sim.npols, sim.nsteps), dtype=np.float64)
    xu = beads.xu

    for step in range(sim.nsteps):
        k = 0
        for n in range(sim.npols):
            spiral_number[n, step] = np.abs(\
                calc_spiral_num_per_fil(\
                xu[step,0,k:k+sim.nbpp[n]],\
                xu[step,1,k:k+sim.nbpp[n]], \
                sim.nbpp[n]))
            k += sim.nbpp[n]

    kurtosis_spiral_number = kurtosis(spiral_number, axis=1, fisher=True)
    glob_spiral_number = np.mean(kurtosis_spiral_number)
