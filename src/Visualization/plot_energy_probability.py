...
""" Plot probability distribution of velocities/energy"""
...
def select_data(data):
    ...
    hist2, edges2 = np.histogram(yd2, bins=50, normed=True)
    xd = edges[:-1] + np.diff(edges)/2.
    xd2 = edges2[:-1] + np.diff(edges2)/2.

    return yd, yd2, xd, xd2, hist, hist2, simd
...
def plot_analysis(data, sep, savepdf, *args):
    ...
    vx, vy, xedges, yedges, xhist, yhist, sim = select_data(data)
    gsim = sim
    ...
    popt, pcov = curve_fit(gaussian, yedges, yhist)
    yfit = gaussian(yedges, popt[0])
    print popt[0]

    sns.distplot(vx, norm_hist=True, ax=ax0, label=r"$v_x$")
    sns.distplot(vy, norm_hist=True, ax=ax0, label=r"$v_y$")
    ...
    #                 linewidth=2.0)
    line1 = ax0.plot(yedges, yfit, "--", c='black', \
    ...
    ax0.set_xlabel(r'$v_{i}$', fontsize=30)
    ax0.set_ylabel(r'PDF($v_{i}$)', fontsize=30)
