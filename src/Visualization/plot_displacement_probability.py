...
""" Plot displacement squared distribution"""
...
class PlotProps:
    ...
    def __init__(self):
        ...
        self.read_fnc = read_write.read_1d_analysis_data
        self.num_ticks = 5
...
def select_data(data, keys):
    ...
    for j, key in enumerate(keys):
        ...
        yd[key] = data[key].data[0]/simd[key].avg_radius
        xd[key] = data[key].data[0]
    return xd, yd, simd

##############################################################################

def select_single_data(data):
    """ select and/or normalize the data for plotting"""

    simd = data.sim
    yd = data.data/simd.avg_radius**2
...
def plot_analysis(data, sep, savepdf, *args):
    ...
    for key in keys:
        ...
        line0 = plt.hist(y, normed=True, label=label, bins=50, alpha=0.8)
        #line0 = plt.hist(y, normed=True, label=label, bins=20, alpha=0.8)
    ...
    ax0.set_xlabel(r'$r^2/R^2$', fontsize=30)
    ax0.set_ylabel(r'PDF($r^2/R^2$)', fontsize=30)
...
@plotter.plot_single
def plot_single_analysis(data, sep, savepdf, *args):
    """ plot the analysis data specific for this analysis"""
    ...
    vx, sim = select_single_data(data)
    gsim = sim
    avg_radius = gsim.avg_radius
    data_separator.assign_physicalvalues(sep.fixed_param, gsim)
    ...
    #popt, pcov = curve_fit(gaussian, xedges, xhist)
    #yfit = gaussian(xedges, popt[0])
    #print popt[0]

    sns.distplot(vx, norm_hist=True, ax=ax0)
    ...
    #                 linewidth=2.0)
    #line1 = ax0.plot(xedges, xfit, "--", c='black', \
    #                 label="_nolegend_", linewidth=2.0)
...
def main():
    ...
    plot_single_analysis(separator.data, separator, args.savepdf)
