...
""" Plot spiral number as a function of a simulation parameter"""
...
def select_data(data, lg_keys, ct_keys):
    ...
    for j, lkey in enumerate(lg_keys):
        ...
        for i, ckey in enumerate(ct_keys):
            ...
            #if (data[key].sim.eps != -1 and data[key].sim.eps != 3.0):
            if (data[key].sim.density != -1):
                yl.append(data[key].data)
                #xl.append(data[key].sim.pe)
                #xl.append(data[key].sim.eps)
                xl.append(data[key].sim.density)
                print lkey, ckey, data[key].data
        xd[lkey] = np.array(xl)
        yd[lkey] = np.array(yl)
        simd[lkey] = sims
...
def plot_analysis(data, sep, savepdf, *args):
    ...
    #ax0.set_xlabel(r'$\epsilon$', fontsize=30)
    ax0.set_xlabel(r'$\phi$', fontsize=30)
    ax0.set_ylabel(r'$\langle |s|\rangle$', fontsize=30)
    ...
    ax0.legend(bbox_to_anchor=(0.65, 0., 0.65, 1.), loc=1, borderaxespad=0., \
