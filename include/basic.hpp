...
template<class T>
inline void allocate_1d_array (T *&arr, int ndata) {
  /* allocate and initialize 1D array */
...
template<class T>
inline void allocate_2d_array (T **&arr, int nrowdata, int ncoldata) {
  /* allocate and initialize 2D array */
  
  arr = new T*[nrowdata];
  for (int n = 0; n < nrowdata; n++) 
    arr[n] = new T[ncoldata];
  for (int nrow = 0; nrow < nrowdata; nrow++) {
    for (int ncol = 0; ncol < ncoldata; ncol++) {
      arr[nrow][ncol] = 0;
    }
  }
